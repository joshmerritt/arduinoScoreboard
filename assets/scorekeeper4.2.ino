...
//  slidePuck(CRGB::Purple);
  ...
  for(int k = 0; k < NUM_LEDS; k++) {
    ...
    twentyOneToWin = analogRead(A5);
  Serial.println("twentyOneToWin");
  Serial.println(twentyOneToWin);
  if(twentyOneToWin) {
    winningScore = 21;
        leds[5] = CRGB::Purple;
    ...
    winningScore = 15; 
    for(int j = 0; j < 13; j++) {
      leds[blueDigits[5][j]] = CRGB::Purple;
      leds[blueDigits[10][j]] = CRGB::Purple;
  }
  }
  FastLED.show();
  delay(5000);
...
void loop() {
  ...
  if( (margin > 1) && ( (redScore >= winningScore) || (blueScore >= winningScore) ) ){
...
void readButtons() {
  ...
  soundOff = analogRead(A4);
  Serial.println("switchA4");
  Serial.println(analogRead(A4));
  Serial.println(soundOff);
    ...
    if(prevButtonMillis[j] - currentMillis > buttonInterval){
      if(buttons[j] != prevButtons[j]){
        if(buttons[j] == 1){
          switch(j) {
            case 0:
              if(buttons[3] == 1) resetPushed = true;
              blueScore++;
              if(soundOff == 0) {
                digitalWrite(5,HIGH);
                FastLED.delay(125);
