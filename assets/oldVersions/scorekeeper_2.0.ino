...
unsigned long buttonInterval = 40; // number of millisecs between button readings
...
void setup() {
  ...
  numberButtons = sizeof( buttonPins ) / sizeof( int );
...
//  Serial.println(previousButtonMillis);
  ...
  redPlus = digitalRead(11);
  redMinus = digitalRead(10);
  blueMinus = digitalRead(9);
  bluePlus = digitalRead(8);
  previousButtonMillis = currentMillis;
    if (redPlus == HIGH && redPlus != prevRedPlus) {
        redScore++;
    }
    if(redMinus == HIGH) {
        redScore--;
